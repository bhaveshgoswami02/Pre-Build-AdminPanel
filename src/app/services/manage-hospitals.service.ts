...
export class ManageHospitalsService {
  ...
  add(data, Img) {
    ...
    this.db.collection(this.collection).doc(data.hospitalId).get().subscribe(res => {
      if (res.exists) {
        this.common.showToast("warning", "Hospital Already Exist", "")
        this.common.stopLoader()
      }
      else {
        return this.db.collection(this.collection).doc(data.hospitalId).set(data).then(res => {
          let path = this.collection + "/" + data.hospitalId + "/" + this.collection
          this.router.navigateByUrl("/" + this.collection)
          if (Img) {
            this.storage.upload(path, Img).then(imgUrl => {
              this.update(data.hospitalId, { imgUrl: imgUrl })
            }).catch(err => {
              console.log(err)
            })
          }
